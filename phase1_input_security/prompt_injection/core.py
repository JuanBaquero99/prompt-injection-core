...
class MLDetectorAdapter(Detector):
    ...
    def detect(self, prompt: str) -> List[Detection]:
        ...
        # Usar probabilidad directa en lugar de prediction categorica
        prob_malicious = result['prob_malicious']
        
        # Umbrales ajustados basados en el análisis de casos reales
        if prob_malicious >= 0.494:  # Umbral muy bajo para detectar casos borderline
            if prob_malicious >= 0.75:
                severity = "HIGH"
            elif prob_malicious >= 0.65:
                severity = "MEDIUM"
            else:
                severity = "LOW"
